...
class NuevaReserva(object):
    ...
    def capturarDatos(self):
        ...
        if self.fechaFinal:
            try:
                self.fechaFinalDatetime = datetime.strptime(self.fechaFinal, '%d/%m/%Y')
                self.fechaFinalDate = self.fechaFinalDatetime.date()
                self.fechaFinalSql = self.fechaFinalDatetime.strftime('%Y/%m/%d')
                contador+=1
                # Continuar con el código que sigue después de la conversión
            except ValueError:
        ...
        if(contador==4):
            if(self.fechaInicialDate<self.fecha_Actual):
                    ...
                    return
            if(self.fechaFinalDate<self.fechaInicialDate):
                self.lanzarPanelInformativo("La fecha final de reserva no puede ser anterior a la fecha inicial de la misma.")
                return
